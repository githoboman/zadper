...
const REASON_COPY: Record<string, string> = {
  ...
  payment_rejected:
    "Zadper rejected this payment. Request a fresh quote and try again.",
  ...
  payment_verifier_unconfigured:
    "Zadper can't verify payments yet because its payment verifier isn't configured.",
  x402_asset_package_hash_required:
    "Zadper can't take payments yet because its payment asset isn't configured.",
  payee_address_required:
    "Zadper can't take payments yet because no payee account is configured.",
  agent_pay_registry_package_hash_required:
    "Zadper can't record decisions on Bot Chain yet because its registry contract isn't configured.",
  agent_pay_registry_package_hash_invalid:
    "Zadper's registry contract address is invalid. It must be a 64-hex-character package hash.",
  agent_pay_registry_configuration_required:
    "Zadper can't record decisions on Bot Chain yet because its registry setup is incomplete.",
  x402_asset_package_hash_must_be_64_hex_chars:
    "Zadper's payment asset is misconfigured. The package hash must be 64 hex characters.",
  payee_address_must_be_00_plus_64_hex_chars:
    "Zadper's payee account is misconfigured. It must be \"00\" followed by 64 hex characters.",
  payment_requirement_required:
    "Zadper can't take payments yet because no payment requirement is configured.",
  payment_configuration_required:
    "Zadper can't take payments yet because its payment settings are incomplete.",
...
const SUBSTRING_COPY: Array<{ match: string; copy: string }> = [
  {
    ...
    copy: "Zadper isn't set up to run live checks yet. The operator still needs to configure a Bot Chain signing key."
  },
  {
    match: "AGENT_PAY_REGISTRY_PACKAGE_HASH",
    copy: "Zadper can't record decisions on Bot Chain yet because its registry contract isn't configured."
  },
  {
    match: "CASPER_RPC_URL",
    copy: "Zadper has no Bot Chain RPC endpoint configured, so it can't confirm on-chain activity."
  },
  {
  ...
  },
  {
    ...
    copy: "Can't reach Zadper. Check that its services are running, then try again."
  },
  ...
  // copy is never clobbered.
  {
    ...
    copy: "Zadper returned an unexpected response. Check that its services are running, then try again."
  },
  {
